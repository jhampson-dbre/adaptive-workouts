...
const deriveSlotConfig = (config, slot) => {
  if (![1, 2].includes(slot)) throw new Error('Expected --slot 1 or 2');
  const offset = slot - 1; const emulators = config.emulators; const ports = [emulators.auth.port, emulators.firestore.port, emulators.firestore.websocketPort, emulators.hub.port, emulators.logging.port];
  const vite = Math.max(...ports) + 2 + offset;
  return { config: { ...config, firestore: { ...config.firestore, rules: path.resolve(config.firestore.rules) }, emulators: { ...emulators, auth: { ...emulators.auth, port: emulators.auth.port + offset }, firestore: { ...emulators.firestore, port: emulators.firestore.port + offset, websocketPort: emulators.firestore.websocketPort + offset }, hub: { ...emulators.hub, port: emulators.hub.port + offset }, logging: { ...emulators.logging, port: emulators.logging.port + offset } } }, vite };
};
export async function runPrivateAccessScenario({ scenario, viewport, slot, deps = {} }) {
  const start = deps.startEmulatorStack ?? startEmulatorStack; const spawn = deps.spawn ?? spawnOwnedProcess; const temp = deps.mkdtemp ?? mkdtemp; const remove = deps.rm ?? rm; const write = deps.writeFile ?? writeFile; const createServer = deps.createControlServer ?? createControlServer; const makeSession = deps.randomUUID ?? randomUUID; const terminate = deps.terminateProcessTree ?? terminateProcessTree; const registryPath = deps.registry ?? registry; const stageStart = deps.stageScenarioStart ?? stageScenarioStart; const applyAction = deps.applyScenarioAction ?? applyScenarioAction; const portPreflight = deps.preflightPorts ?? preflightPorts;
  const preflight = preflightAccessScenario({ scenario, manifest: accessScenarioManifest }); const { config, vite: vitePort } = deriveSlotConfig(JSON.parse(await readFile('firebase.emulator-test.json', 'utf8')), slot); await portPreflight([{ name: 'Vite', host: '127.0.0.1', port: vitePort }]); const temporaryRoot = await temp(path.join(os.tmpdir(), 'private-access-')); const scratchDirectory = path.join(temporaryRoot, 'scratch'); const configPath = path.join(temporaryRoot, 'firebase.emulator-test.json'); const session = makeSession();
  ...
  try {
    ...
    stack = await start({ configPath, projectId: 'demo-project', profile: 'scratch', seedProfile: 'test', scratchDirectory });
    ...
    const origin = `http://127.0.0.1:${vitePort}`; server = createServer({ sessionId: session, origin, onAction: async ({ action }) => { stageAccessAction({ manifest: accessScenarioManifest, scenario, action }); return applyAction({ action, projectId: 'demo-project', hosts: stack.hosts }); }, onStop: requestStop });
    await waitForListening(server);
    const controlPort = server.address().port; const controlUrl = `http://127.0.0.1:${controlPort}/sessions/${session}`;
    vite = spawn(process.execPath, [path.resolve('node_modules/vite/bin/vite.js'), '--host', '127.0.0.1', '--port', String(vitePort), '--strictPort', '--mode', 'baseline'], { cwd: process.cwd(), stdio: 'ignore', env: { ...process.env, VITE_FIREBASE_AUTH_EMULATOR_HOST: stack.hosts.auth, VITE_FIRESTORE_EMULATOR_HOST: stack.hosts.firestore, VITE_ACCESS_SCENARIO_CONTROL_SESSION: session, VITE_ACCESS_SCENARIO_CONTROL_URL: controlUrl } });
    const metadata = { ...preflight, stagedIdentity, slot, session, viewport, url: origin, ports: { auth: config.emulators.auth.port, firestore: config.emulators.firestore.port, firestoreWebsocket: config.emulators.firestore.websocketPort, hub: config.emulators.hub.port, logging: config.emulators.logging.port, vite: vitePort, control: controlPort }, hosts: stack.hosts, artifactRevision: accessScenarioManifest.artifactRevision, fixtureRevision: accessScenarioManifest.fixtureRevision, authRevision: accessScenarioManifest.authRevision };
