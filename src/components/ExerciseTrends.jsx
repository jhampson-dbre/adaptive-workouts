...
function TrendPlot({ trend, index, window, onPoint }) {
  ...
  const x = item => 20 + ((new Date(`${visibleCalendarDate(item.date)}T00:00:00Z`).getTime() - first) / span) * 280;
  ...
  const spread = maximum - minimum || Math.max(Math.abs(maximum) * 0.1, 1); const padding = spread * 0.1;
  const lower = Math.max(0, minimum - padding); const upper = maximum + padding;
  const y = value => 132 - ((value - lower) / (upper - lower)) * 104;
  const bound = value => Number.isInteger(value) ? value : value.toFixed(1); const unit = trend.trackingMode === 'weighted' ? 'lb' : 'reps';
  ...
  return <figure className="trend-plot"><figcaption className="trend-plot-key">{series.map(item => <span key={item.key}>{item.label}: {item.dash ? item.marker === 'square' ? 'dashed square' : 'dotted diamond' : 'solid circle'}</span>)}</figcaption><div className="trend-plot-bounds">Plot scale: {bound(lower)} to {bound(upper)} {unit}</div><svg data-testid="trend-plot" aria-hidden="true" viewBox="0 0 320 152" preserveAspectRatio="xMidYMid meet" onPointerDown={chooseNearest}><line className="trend-selected-rail" x1={x(points[index])} x2={x(points[index])} y1="12" y2="136" />{series.map(item => <path key={item.key} className="trend-line" d={path(item.key)} strokeDasharray={item.dash} />)}{points.map((item, itemIndex) => <g key={item.workoutId} data-point-index={itemIndex} data-point-x={x(item)}>{series.map(seriesItem => marker(item, itemIndex, seriesItem))}</g>)}</svg><div className="trend-plot-dates"><span>{formatDate(window.startDate)}</span><span>{formatDate(window.endDate)}</span></div></figure>;
}
