...
export default function ExerciseTrends({ loadRange }) {
  ...
  if (!selected) return <section className="exercise-trends exercise-discovery" aria-labelledby="exercises-heading">
...
function TrendDetail({ summaryRef, trend, point, index, window, onPoint }) {
  ...
  return <><TrendPlot trend={trend} index={index} window={window} onPoint={onPoint} /><section className="trend-overview" aria-labelledby="trend-summary-heading"><h4 ref={summaryRef} id="trend-summary-heading" tabIndex="-1">Performance overview</h4><dl className="trend-stats">{trend.trackingMode === 'weighted' ? <><div className="trend-stat"><dt>Latest volume:</dt><dd>{latest.value} lb</dd></div><div className="trend-stat"><dt>Range high:</dt><dd>{Math.max(...points.map(item => item.value))} lb</dd></div>{previous && <div className="trend-stat trend-stat-supporting"><dt>Since {formatShortDate(previous.date)}:</dt><dd>{volumeChange(point.value, previous.value)}</dd></div>}</> : <><div className="trend-stat"><dt>Latest totals:</dt><dd>{pointLabel(latest)}</dd></div><div className="trend-stat"><dt>Highest full reps in one workout:</dt><dd>{Math.max(...points.map(item => item.fullReps))}</dd></div><div className="trend-stat"><dt>Highest assisted reps in one workout:</dt><dd>{Math.max(...points.map(item => item.assistedReps))}</dd></div><div className="trend-stat"><dt>Highest eccentric reps in one workout:</dt><dd>{Math.max(...points.map(item => item.eccentricReps))}</dd></div>{previous && <div className="trend-stat trend-stat-supporting"><dt>Since {formatShortDate(previous.date)}:</dt><dd>Full {signed(point.fullReps - previous.fullReps)} · Assisted {signed(point.assistedReps - previous.assistedReps)} · Eccentric {signed(point.eccentricReps - previous.eccentricReps)}</dd></div>}</>}{!one && <div className="trend-stat trend-stat-supporting"><dt>Sessions:</dt><dd>{points.length}</dd></div>}</dl>{one && <p className="trend-overview-note">One recorded workout in this range.</p>}</section><div className="trend-selection"><label className="trend-scrubber">Recorded workout {index + 1} of {points.length}: {formatDate(point.date)} · {pointLabel(point)}<input aria-label="Recorded workout" type="range" min="0" max={points.length - 1} value={index} onChange={event => onPoint(Number(event.target.value))} onKeyDown={move} /></label><p className="trend-selection-confirmation" aria-live="polite">Selected {formatDate(point.date)}: {pointLabel(point)}</p></div><section className="trend-confirmed-sets" aria-labelledby="confirmed-set-heading"><h4 id="confirmed-set-heading">Sets in selected workout</h4><ul className="trend-confirmed-list">{point.confirmedSets.map((set, setIndex) => <li key={setIndex}>{trend.trackingMode === 'weighted' ? `${set.actualWeight} lb × ${set.actualReps} reps` : `Full ${set.fullReps} · Assisted ${set.assistedReps} · Eccentric ${set.eccentricReps}`}</li>)}</ul></section></>;
}
