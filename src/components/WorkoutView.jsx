...
function recordedWorkText(exercises) {
  const confirmedSets = exercises.reduce((total, exercise) => total + (exercise.setRecords?.filter(record => record.completed).length ?? 0), 0);
  const confirmedExercises = exercises.filter(exercise => exercise.setRecords?.some(record => record.completed) || exercise.completed).length;
  return `${confirmedSets} ${confirmedSets === 1 ? 'set' : 'sets'} and ${confirmedExercises} ${confirmedExercises === 1 ? 'exercise' : 'exercises'}`;
}
...
function RecordedExercises({ exercises, summary }) {
  return <details className="summary-remaining summary-recorded"><summary>{summary}</summary><ul>{exercises.filter(exercise => exercise.setRecords?.some(record => record.completed) || exercise.completed).map((exercise, index) => {
...
function PlannedWorkNotRecorded({ exercises }) {
  const unconfirmed = exercises.filter(exercise => exercise.setRecords?.some(record => !record.completed) || (!exercise.setRecords && !exercise.completed));
  return unconfirmed.length > 0 && <details className="summary-remaining"><summary>Planned work not recorded</summary><ul>{unconfirmed.map((exercise, index) => <li key={`${exercise.id}-${index}`}>{exercise.name}</li>)}</ul></details>;
}
...
function WorkoutSummary({ candidate, phaseTargets, isSaving, saveError, saveStatus, blockedConflict, onBack, onSave, onKeepPending, onExit, summaryRef }) {
  const recordedWork = recordedWorkText(candidate.exercises);
  ...
  return (
    <section className="workout-summary" role="region" aria-label="Workout summary">
      ...
      <RecordedExercises exercises={candidate.exercises} summary="Review recorded exercises" />
    </section>
  );
}
...
function SetRow({ exercise, exerciseIndex, setIndex, started, activeTimer, activeOwnerName, now, dispatch, error, onError, onClearError, onStart, onConfirm, onCancel, startRef, restRecord, claimFinalRestCue }) {
  ...
  useEffect(() => { if (isActive) activeControlRef.current?.focus(); }, [isActive]);
  const start = () => {
  ...
  return <section className={`set-row ${status}${isActive ? ' active-work' : ''}`} aria-label={prefix}>
    ...
    <div className="set-timing">
      {isActive ? <><span className="work-timer">Work: {formatTime(calculateElapsedSeconds(activeTimer.startedAt, now))}</span><button ref={activeControlRef} type="button" aria-label={`${prefix} confirm`} aria-describedby={error ? errorId : undefined} onClick={() => onConfirm(exerciseIndex, setIndex)}>Confirm attempt</button><button type="button" aria-label={`${prefix} cancel`} onClick={() => onCancel(exerciseIndex, setIndex)}>Cancel timer</button></>
        : status === 'ready' ? <><button type="button" ref={element => { activeControlRef.current = element; startRef(element); }} className={showRestCue ? 'rest-final-cue' : undefined} aria-label={`${prefix} start`} disabled={!started} aria-describedby={error ? errorId : (!started ? 'workout-start-help' : undefined)} onClick={start}>{isResting ? 'Start set early' : 'Start set'}</button>{restRecord && <span className="superset-rest-status rest-timer"><RestReadout record={restRecord} now={now} /></span>}</>
...
export default function WorkoutView({ session, sessionState, onFinish, onComplete, onDiscard, onBackToPlan, onResume, preference, onSavePreference, onStarted, onDismissPreference }) {
  ...
  const pendingOrderFocusRef = useRef(null);
  const orderBlockRefs = useRef({});
  const saveOutcomeRef = useRef(null);
  ...
  activeWorkTimerRef.current = activeWorkout.activeWorkTimer;
  const started = activeWorkout.workoutStartedAt !== null;
  const announcedMainWorkoutRef = useRef(started && activeWorkout.phase === 'performance');
  const hasUnfinishedSets = activeWorkout.exercises.some(exercise => exercise.setRecords.some(record => !record.completed));
  const orderBlocks = activeWorkout.exercises.reduce((blocks, exercise) => {
    ...
    blocks.push({ ids: group, exercises: group.map(id => activeWorkout.exercises.find(item => item.occurrenceId === id)).filter(Boolean) }); return blocks;
  }, []);
  const orderRule = { blocks: orderBlocks.map(block => ({ exerciseIds: block.exercises.map(exercise => exercise.id) })) };
  const plannedSeconds = Object.values(sessionState?.phaseTargets ?? {}).reduce((total, seconds) => total + (Number.isFinite(seconds) ? seconds : 0), 0);
  const orderSummaryNames = orderBlocks.slice(0, 3).map(block => block.exercises.length > 1
    ? `Superset: ${block.exercises.map(exercise => exercise.name).join(' / ')}`
    : block.exercises[0]?.name
  ).filter(Boolean);
  const orderSummary = `${orderSummaryNames.join(', ')}${orderBlocks.length > 3 ? `, and ${orderBlocks.length - 3} more` : ''}`;
  const counts = workoutCounts(activeWorkout.exercises);
  const dirtyOrder = Boolean(preference?.baseline) && JSON.stringify(orderRule) !== JSON.stringify(preference.baseline);
  const orderBusy = ['pending', 'indeterminate', 'clearing'].includes(preference?.operation?.state);
  const moveBlock = async (block, direction, focusKey) => {
    pendingOrderFocusRef.current = { key: focusKey, direction };
    const accepted = await session.action({ type: 'moveGeneratedBlock', occurrenceId: block.exercises[0].occurrenceId, direction });
    if (!accepted) { pendingOrderFocusRef.current = null; return; }
    setRecoveryAcknowledgement('');
    const currentPosition = orderBlocks.indexOf(block);
    const next = direction === 'earlier' ? currentPosition : currentPosition + 2;
    setOrderAnnouncement(`${block.exercises.map(exercise => exercise.name).join(' / ')} moved to position ${next} of ${orderBlocks.length}. This change is for this workout only.`);
  };
  const journeyStep = activeWorkout.phase === 'cooldown'
    ? 'Cooldown'
    : activeWorkout.phase === 'review' ? 'Review'
      : started || ['warmup', 'performance'].includes(activeWorkout.phase) ? 'Workout' : 'Plan';
  const phaseTitle = started && activeWorkout.phase === 'generated'
    ? 'Main workout'
    : { generated: 'Review your workout', warmup: 'Warmup', performance: 'Main workout', cooldown: 'Cooldown', review: 'Review', cancelled: 'Workout cancelled' }[activeWorkout.phase] ?? 'Workout';
  ...
  useEffect(() => {
    ...
    if (!pending) return;
    const block = orderBlockRefs.current[pending.key];
    const control = block?.querySelector(`button[data-order-direction="${pending.direction}"]`);
    (control && !control.disabled ? control : block)?.focus();
  ...
  const handleStartSet = (exerciseIndex, setIndex) => {
    ...
    const action = { type: 'startSet', exerciseIndex, setIndex, timestamp: acceptDisplayTime(Date.now()) };
    if (!announcedMainWorkoutRef.current) void (async () => {
      if (await session.action(action)) {
        announcedMainWorkoutRef.current = true;
        setRecoveryAcknowledgement('Main workout started.');
      }
    })();
  ...
  const copyWorkout = async () => {
    try {
      ...
      await navigator.clipboard.writeText(formatWorkoutClipboard(sessionState?.savedReceipt ?? finishCandidate));
  ...
  const renderSetRow = (exercise, exerciseIndex, setIndex) => <SetRow
    ...
    setIndex={setIndex}
    started={started}
    activeTimer={activeWorkout.activeWorkTimer}
    activeOwnerName={activeWorkout.activeWorkTimer ? activeWorkout.exercises[activeWorkout.activeWorkTimer.exerciseIndex].name : ''}
    now={now}
    dispatch={action => action.type === 'undoSet' ? handleUndo(action.exerciseIndex, action.setIndex) : dispatch(action)}
    ...
    onCancel={(index, set) => {
      dispatch({ type: 'cancelSet', exerciseIndex: index, setIndex: set });
      clearExerciseError(index);
      clearErrorsBlockedBy(index, set);
      setFinishError('');
    }}
  ...
  const exerciseRow = (exercise, exerciseIndex, orderBlock, orderIndex, reserveOrderRail = false) => {
      ...
      const superset = supersetFor(activeWorkout, exercise);
      const next = superset && nextSupersetSet(activeWorkout, superset);
      const supersetContext = started && superset && `Superset ${superset.occurrenceIds.indexOf(exercise.occurrenceId) + 1} of ${superset.occurrenceIds.length}${next?.exerciseIndex === exerciseIndex ? ' · Next' : ''}`;
      ...
      return <li key={exercise.occurrenceId || `${exercise.id}-${exerciseIndex}`} className={`${confirmed === exercise.setRecords.length ? 'completed ' : ''}${isExpanded ? 'selected' : ''}`} ref={orderBlock ? node => { orderBlockRefs.current[orderFocusKey] = node; } : undefined} tabIndex={orderBlock ? '-1' : undefined}>
        <div className="exercise-row">
          ...
          {orderBlock ? renderOrderActions(orderBlock, orderIndex, orderFocusKey) : reserveOrderRail && <span className="exercise-order-actions exercise-order-spacer" aria-hidden="true" />}
        </div>
  ...
  const exerciseList = <>
    ...
    <ul className="workout-checklist">{started ? orderBlocks.map(orderBlock => {
      ...
      const memberListId = `superset-${orderFocusKey}-members`;
      return <li key={orderFocusKey} className="workout-order-block superset-order-block order-block" role="group" aria-label={`Superset: ${names}, ${positions}, ${orderBlock.exercises.length} exercises, ${plannedSets} planned sets`} ref={node => { orderBlockRefs.current[orderFocusKey] = node; }} tabIndex="-1">
        <div className="superset-order-header exercise-row">
          <button type="button" className="superset-order-disclosure" aria-expanded={isExpanded} aria-controls={isExpanded ? memberListId : undefined} aria-label={`Superset: ${names}, ${positions}, ${orderBlock.exercises.length} exercises, ${plannedSets} planned sets, ${isExpanded ? 'collapse' : 'expand'}`} onClick={() => setExpandedSupersets(current => ({ ...current, [orderFocusKey]: !isExpanded }))}>
            <span className="superset-order-range" aria-hidden="true">{!isExpanded && <><span className="exercise-number">{String(firstPosition).padStart(2, '0')}</span><span className="superset-order-range-rule" aria-hidden="true" /><span className="exercise-number">{String(lastPosition).padStart(2, '0')}</span></>}</span>
            <span className="superset-order-copy"><span>{isExpanded ? `SUPERSET · ${positionRange}` : 'Superset'}</span><span className="exercise-name"><strong>{names}</strong> <small>{orderBlock.exercises.length} exercises</small></span></span><span className="exercise-status"><span>{plannedSets} planned sets ·</span><span>{isExpanded ? 'Collapse' : 'Expand'}</span></span>
          </button>
          {renderOrderActions(orderBlock, orderIndex, orderFocusKey)}
        </div>
        {isExpanded && <ul id={memberListId} className="superset-exercise-rows">{orderBlock.exercises.map(exercise => exerciseRow(exercise, activeWorkout.exercises.indexOf(exercise), undefined, undefined, true))}</ul>}
      </li>;
    })}</ul>
    {!started && <section className="order-save-controls" aria-label="Exercise order saving">
      {dirtyOrder && <><p>Order changes apply only to today unless you save them.</p><button type="button" disabled={orderBusy} onClick={() => onSavePreference?.(orderRule, Object.fromEntries(activeWorkout.exercises.map(exercise => [exercise.id, exercise.name])))}>{preference?.operation?.state === 'pending' || preference?.operation?.state === 'indeterminate' ? 'Saving order…' : preference?.operation?.state === 'failure' ? 'Try saving this exercise order again' : 'Save order for future workouts'}</button></>}
  ...
  useEffect(() => {
    if (sessionState?.status === 'saved') setRecoveryAcknowledgement('Workout saved.');
  }, [sessionState?.status]);
  useEffect(() => { if (sessionState?.status === 'saved') savedRef.current?.focus(); }, [sessionState?.status]);

  if (showingRecovery && sessionState?.status !== 'saved') {
    ...
    return <div className="workout-view recovery-view">
      <section className="recovery-panel" role="region" aria-label="Interrupted workout">
        ...
        {!checking && <div className="recovery-actions">
          ...
          <button className="recovery-secondary" type="button" onClick={async () => { if (discardable) { await session.discard(); (onDiscard ?? onFinish)?.(); } else { await session.exit(); (onComplete ?? onFinish)?.(); } }}>{discardable ? 'Discard workout' : 'Exit to Plan'}</button>
        </div>}
      </section>
    </div>;
  }
  if (sessionState?.status === 'saved') {
    const receipt = sessionState.savedReceipt;
    const returnToPlan = () => {
