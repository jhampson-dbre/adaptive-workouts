...
export default function Generator({
  ...
  const handleToggleGroup = group => {
    if (recoveryApplyRef.current) return;
    invalidateRecovery();
    setDraftGroups(current => current.includes(group) ? current.filter(item => item !== group) : [...current, group]);
  };
  const handleTimeChange = value => { if (!recoveryApplyRef.current) { invalidateRecovery(); setTimeBudget(value); } };
  ...
  const applyOption = option => {
    ...
    recoveryApplyRef.current = true; setIsApplyingRecovery(true);
    void applyWorkout(option.workout, recovery.capture, adjusted).finally(() => {
      recoveryApplyRef.current = false; setIsApplyingRecovery(false);
    });
  };
  ...
  const recoveryArea = recovery && <section className="no-fit-recovery" aria-busy={recovery.kind === 'checking' || isApplyingRecovery ? 'true' : undefined}>
    {recovery.kind === 'checking' ? <p role="status">Checking other muscle groups…</p> : <>
      ...
      {recovery.kind === 'options' && recovery.options.length > 1 && <>
        ...
        <fieldset className="recovery-options" role="radiogroup" aria-label="Muscle groups to include"><legend className="visually-hidden">Muscle groups to include</legend>{recovery.options.slice(0, showAll ? undefined : 3).map((option, index) => <label key={groupKey(option.groups)}><input type="radio" name="recovery-option" checked={selectedOption === index} disabled={isApplyingRecovery} onChange={() => setSelectedOption(index)} />{groupList(option.groups)}</label>)}</fieldset>
  ...
  const groupControls = groups => groups.map(group => <label key={group} className="area-status-button"><span>{group}</span><span className="area-state">{draftGroups.includes(group) ? 'Skip' : 'Include'}</span><input type="checkbox" checked={draftGroups.includes(group)} disabled={isApplyingRecovery} onChange={() => handleToggleGroup(group)} /></label>);
  const editor = <>
  ...
  return <div className="generator">
    ...
    <div className="slider-container"><label htmlFor="time-slider">Time available<span className="slider-value">{timeBudget} <small>min</small></span></label><input ref={timeRef} id="time-slider" type="range" min="15" max="120" step="5" value={timeBudget} disabled={isApplyingRecovery} onChange={event => handleTimeChange(Number(event.target.value))} className="slider" /><div className="time-stepper" aria-label="Precise time adjustment"><button type="button" aria-label="Decrease time by 5 minutes" disabled={timeBudget <= 15 || isApplyingRecovery} onClick={() => handleTimeChange(Math.max(15, timeBudget - 5))}>−5 min</button><button type="button" aria-label="Increase time by 5 minutes" disabled={timeBudget >= 120 || isApplyingRecovery} onClick={() => handleTimeChange(Math.min(120, timeBudget + 5))}>+5 min</button></div></div>
