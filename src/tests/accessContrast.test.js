...
const errorForeground = indexCss.match(/\.error-message\s*\{[^}]*color:\s*([^;]+)/i)?.[1].trim().toLowerCase();
const errorBackground = indexCss.match(/\.error-message\s*\{[^}]*background:\s*([^;]+)/i)?.[1].trim().toLowerCase();
