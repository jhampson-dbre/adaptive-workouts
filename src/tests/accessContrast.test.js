...
const errorForeground = resolveColor(indexCss.match(/\.error-message\s*\{[^}]*color:\s*([^;]+)/i)?.[1].trim().toLowerCase());
const errorBackground = resolveColor(indexCss.match(/\.error-message\s*\{[^}]*background:\s*([^;]+)/i)?.[1].trim().toLowerCase());
