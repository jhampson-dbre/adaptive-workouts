...
test('renders a pre-start superset as a collapsed summary with separate shared order and disclosure controls', () => {
  ...
  const disclosure = within(supersetBlock).getByRole('button', { name: /Superset: Long Row Exercise Name and Long Press Exercise Name.*2 exercises.*2 planned sets.*expand/i });
...
test('presents one guided action before the optional exercise list', () => {
  ...
  const alternateExercise = screen.getByRole('button', { name: /Squat, 0 of 1 confirmed/i });
...
test('labels a completed simple exercise factually in Review', () => {
  const activeWorkout = {
    ...
    workoutStartedAt: 1,
    exercises: [
      { id: 'walk', occurrenceId: 'walk:0', name: 'Walk', completed: true, setRecords: [] },
      { id: 'press', occurrenceId: 'press:1', name: 'Press', setRecords: [{ completed: true }] },
    ],
    phaseCandidate: {
      exercises: [
        { id: 'walk', occurrenceId: 'walk:0', name: 'Walk', completed: true, setRecords: [] },
      ...
      phaseActualSeconds: { warmup: 0, performance: 60, cooldown: 0 },
      finishRequestedAtEpochMs: 1000,
    },
  };
...
test('renders the session-produced divergent immutable-save conflict as frozen Review', async () => {
  ...
  await waitFor(() => expect(document.activeElement).toBe(screen.getByRole('heading', { level: 2, name: 'Review' })));
  fireEvent.click(screen.getByRole('button', { name: 'Save workout' }));
