...
- Before final integration of a materially changed, stabilized green in-scope diff, run one bounded `$ponytail-review` proposal pass. Capture one-line proposals and net estimate; treat findings as hypotheses and validate each through callers, state, trust, security, data loss, concurrency, accessibility, focus, persistence, schema, fingerprint, ordering, determinism, errors, and tests as applicable. Line reduction alone is insufficient: preserve public behavior, persistence/schema/fingerprint compatibility, and error behavior. The final handoff records dispositions, cuts, verification, and routing with concise durable reasons; dispositions are accepted, rejected, or deferred—not a ledger or transcript. Dispatch `$code-simplification` only for an accepted complexity signal; proposals are not edit authority. Route proportionate verification and only review evidence changed by accepted cuts. One pass applies per stabilized evidence set; unchanged evidence never retriggers it.
