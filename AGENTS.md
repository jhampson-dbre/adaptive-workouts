...
- Each tracked behavior-change task starts with one fresh implementation owner: use `ui-implementor` when the primary material risk is required UI's rendered hierarchy, interaction, responsive behavior, or accessibility; otherwise use `implementor`. Reuse that owner for same-task continuations, including review fixes; replace it only when unavailable, unable to continue, unwilling, or the user explicitly changes ownership. Never reuse an implementation owner across Trekker task boundaries. Reproduce bugs, identify the root cause, and inspect plausible same-cause callers before editing; use `$bugfix-issue-class-audit` only when that inspection reveals material scope ambiguity.
