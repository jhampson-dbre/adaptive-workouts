...
- Each tracked behavior-change task starts with one fresh implementation owner: use `ui-implementor` when the primary material risk is required UI's rendered hierarchy, interaction, responsive behavior, or accessibility; otherwise use `implementor`. Reuse that owner for same-task continuations, including the ordinary first review fix; replace it only when unavailable, unable to continue, unwilling, or the user explicitly changes ownership. The coordinator may establish unable-to-continue only when consecutive coordinator-accepted substantive blocking findings show that remediation still fails the same named material safety or correctness boundary; unrelated or new findings, reviewer errors, and unchanged evidence do not qualify. Replacement must leave exactly one active implementation owner for the task. Never reuse an implementation owner across Trekker task boundaries. Reproduce bugs, identify the root cause, and inspect plausible same-cause callers before editing; use `$bugfix-issue-class-audit` only when that inspection reveals material scope ambiguity.
- When `implementor` is primary and its stabilized green task materially changes required UI, dispatch one bounded `ui-implementor` finishing polish pass after implementation evidence and before the goal-first journey or substantive task/epic review. The pass may return no change, does not replace the primary owner, and does not repeat on unchanged evidence. It may make leaf UI refinements that preserve product semantics; findings requiring product behavior, data, state/control-flow, architecture, or an explicitly pinned design decision return to the primary owner. Skip this pass when `ui-implementor` was primary or required UI was not materially changed. Run the goal-first journey against the polished build and reconcile its findings before eligible simplification and substantive task/epic review.
...
- Before first presenting or requesting approval for a material user-visible interaction recommendation or coordinator-authored new or meaningfully revised in-app copy, apply the bounded `$impeccable shape` and/or `$impeccable clarify` advisory in `$ux-quality-gate` and present one synthesized proposal. Apply it again only when new evidence creates a new material UX or copy decision; it is coordinator-owned advice, not a reviewer, approval authority, artifact, or subagent gate.
...
- Classify UI work as `required`, `optional`, or `skip-recorded`. For required UI, during planning the coordinator proposes a proportional set of trainee-centered blind user journeys, obtains user approval, and durably records them in the approved planning artifact or Trekker before implementation. Follow `$ux-quality-gate` for scenario construction, proportionality, and new-evidence additions. Required work keeps its approved scenario artifact and uses a fresh ux-design-reviewer before architecture review when independent design judgment is needed. Architecture retains authority; material architecture changes to the approved UX contract return through UX review before user approval.
...
Use the project role contracts in `docs/agents/` and corresponding `.codex/agents/*.toml`: architecture-design-reviewer, feature-planner-advisor, senior-developer-reviewer, implementor, ui-implementor, code-simplifier, code-reviewer, spec-reviewer, epic-reviewer, ux-design-reviewer, and ux-usability-reviewer. Keep model and reasoning assignments unchanged. Reviewers are read-only unless the coordinator expressly authorizes a patch.
