...
- Viewport and starting state: In-app Chromium at 1280 x 720 and 390 x 844; synthetic production `WorkoutView` fixture with completed Carry, Long Row Exercise Name, and Long Press Exercise Name records and exact row/press superset membership.
- Actions: Save the frozen Review candidate; confirm the saved heading focus and collapsed history; activate Copy workout; inspect feedback, focus, geometry, overflow, console, and the browser clipboard.
- Observed result: `Workout saved` received focus before interaction. Copy appears before Back to plan with the same neutral styling. Native clipboard output was exactly `Carry\n1 set\n0:12\n\nA1. Long Row Exercise Name\n1 set\n0:23\n\nA2. Long Press Exercise Name\n1 set\n0:34`. Success rendered `Workout copied.` in a separate polite status; Copy remained enabled and focused. Copy, Back, and History measured 50.53px high; the phone document measured 390/390 client/scroll width and desktop measured 1280/1280. History remained collapsed (`aria-expanded=false`) and a fresh browser run logged no warnings or errors.
...
- Viewport and starting state: In-app Chromium at 390 x 844 on the same synthetic mixed saved workout; harness rejected the first native `writeText` call and delegated the next call to the browser clipboard.
- Actions: Activate Copy workout once to reject, inspect the error and focus, then activate the same enabled control again.
- Observed result: First activation rendered the sole assertive alert `Couldn’t copy workout. Try again.` while Copy stayed enabled and focused. Retry removed the alert, rendered the polite `Workout copied.` status, retained focus, and wrote the exact standalone/A1/A2 text. The viewport remained 390/390 with no horizontal overflow and History stayed collapsed.
