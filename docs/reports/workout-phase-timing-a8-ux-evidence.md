...
| Backward wall-clock presentation | Fresh IAB-05 capability list again advertised browser-level `visibility` and `viewport` plus tab `pageAssets`; it exposed no clock control or safe Date injection. | `unsupported` | `capability_reason: unsupported-by-harness`; harness/session: Codex in-app browser / `CAP-225-2026-07-22-IAB-05`. Eligible alternatives were source inspection and the mounted production-component fake-clock regression; both were selected because they exercise the same accepted-display-time path for global, phase, work, and rest values. Limitation/residual risk: a backward OS-clock event was not browser-rendered. Fallback: committed WorkoutView regression at `047f0ea`. Reactivation: a browser harness with deterministic wall-clock control. |
...
| T-08 - Clock behavior | Reload after elapsed time in Warmup and Performance; in IAB-05 compare the frozen phase sum before and after a measurable Review gap and Back. | Displayed elapsed time restored across reload. IAB-05 directly rendered the post-Back global total as 3:36, exactly 0:09 + 3:01 + 0:26, despite the Review delay; it advanced only while Cooldown was active. The fresh capability probe exposed no backward-clock control, so committed mounted WorkoutView tests provide the direct fallback that a later backward tick cannot reduce global total or representative phase/work/rest readouts. | `rendered-primary` for phase-sum/Review-gap behavior + committed component clock fallback; `observed-pass`; `evidence-complete-with-residual-capability-risk` for backward OS-clock rendering. |
...
the committed deferred-session/backward-clock component regressions cover the two
harness-unobservable intervals.
