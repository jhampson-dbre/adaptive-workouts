...
- After a green diff, run proportionate verification. Invoke `$code-simplification` only for a concrete complexity signal. Substantive behavior or risk changes receive code and conformance review; mechanical or straightforward low-risk changes may be coordinator-verified with a recorded rationale.
