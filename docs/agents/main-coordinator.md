...
- Before final integration of a materially changed, stabilized green in-scope diff, run one bounded `$ponytail-review` proposal pass. Capture one-line proposals and net estimate; findings are hypotheses, validated through callers, state, trust, security, data loss, concurrency, accessibility, focus, persistence, schema, fingerprint, ordering, determinism, errors, and tests as applicable. Line reduction alone is insufficient: preserve public behavior, persistence/schema/fingerprint compatibility, and error behavior. The final handoff records dispositions, cuts, verification, and routing with concise durable reasons; dispositions are accepted, rejected, or deferred—not a ledger or transcript. Invoke `$code-simplification` only for an accepted complexity signal; proposals are not edit authority. Route proportionate verification and only the review evidence changed by accepted cuts. One pass applies per stabilized evidence set; unchanged evidence never retriggers it. Representative outcomes: accept redundant mirrored state, dead payload plumbing, or duplicate unchanged-input validation; reject security-rule deduplication, shared mutation locks, and two-version fingerprint abstractions when auditability, behavior, or indirection makes the cut unsafe. Substantive behavior or risk changes receive code and conformance review; mechanical or straightforward low-risk changes may be coordinator-verified with a recorded rationale.
